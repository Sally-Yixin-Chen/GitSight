...
try:
    ...
    from .localization import with_project_locales
    ...
    from src.localization import with_project_locales
...
def load_projects() -> List[Dict[str, Any]]:
    ...
    return with_project_locales(load_cached_projects())
...
def project_owner_label(project: Dict[str, Any]) -> str:
    """按“项目——所有者”的顺序生成排行榜展示文本。"""
    full_name = str(project.get("full_name") or project.get("name") or "").strip()
    owner = str(project.get("owner") or "").strip()
    repository = full_name.split("/", 1)[1] if "/" in full_name else full_name
    if not owner and "/" in full_name:
        owner = full_name.split("/", 1)[0].strip()
    if repository and owner:
        return f"{repository} —— {owner}"
    return repository or owner or "未命名项目"


PAGE_STYLE = """
<style>
@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=JetBrains+Mono:wght@500;600&family=Noto+Sans+SC:wght@400;500;600;700&display=swap');
...
body { margin:0; min-width:320px; background:radial-gradient(circle at top right, #dbeafe 0, transparent 34%), var(--bg); color:var(--text); font-family:'DM Sans',system-ui,sans-serif; line-height:1.5; }
...
.brand { display:flex; align-items:center; gap:10px; color:var(--text); font-family:'DM Sans',system-ui,sans-serif; font-weight:700; letter-spacing:-.04em; }
.brand-mark { display:grid; place-items:center; width:34px; height:34px; border:1px solid #93c5fd; border-radius:10px; color:var(--accent); background:#eff6ff; overflow:hidden; } .brand-mark img { width:100%; height:100%; object-fit:contain; }
...
.eyebrow { margin:0 0 9px; color:var(--accent); font:700 .9rem 'DM Sans',system-ui,sans-serif; letter-spacing:.1em; text-transform:uppercase; }
...
.hero-copy { max-width:58ch; margin:16px 0 0; color:var(--muted); font:500 1.2rem/1.8 'Noto Sans SC','Microsoft YaHei',sans-serif; letter-spacing:.02em; }
...
.metric-label { display:block; color:var(--muted); font-size:.9rem; } .metric-value { display:block; margin-top:6px; font:600 1.6rem 'JetBrains Mono',monospace; color:#0f172a; }
...
.topic span { display:flex; align-items:center; min-height:44px; padding:8px 12px; border:1px solid var(--border); border-radius:999px; color:#334155; font:500 .86rem 'DM Sans',system-ui,sans-serif; transition:background .18s ease,border-color .18s ease,color .18s ease; }
...
.table-wrap { overflow-x:auto; margin-top:18px; border:1px solid var(--border); border-radius:14px; } table { width:100%; min-width:780px; border-collapse:collapse; } th { padding:12px 16px; color:var(--muted); background:#f8fafc; font:600 .82rem 'JetBrains Mono',monospace; letter-spacing:.08em; text-align:left; text-transform:uppercase; } td { padding:16px; border-top:1px solid var(--border); } tbody tr { transition:background .18s ease; } tbody tr:hover { background:#f8fbff; }
.rank { display:flex; align-items:center; gap:8px; color:#2563eb; font:600 .92rem 'JetBrains Mono',monospace; } .rank-number { min-width:2ch; } .medal { display:inline-grid; place-items:center; width:28px; height:28px; border-radius:50%; font-family:system-ui,sans-serif; font-size:1.05rem; line-height:1; } .medal-gold { background:#fef3c7; box-shadow:0 0 0 2px #fbbf24; } .medal-silver { background:#f1f5f9; box-shadow:0 0 0 2px #cbd5e1; } .medal-bronze { background:#ffedd5; box-shadow:0 0 0 2px #fb923c; } .project-link { display:flex; flex-direction:column; align-items:flex-start; gap:3px; color:var(--text); font-weight:600; } .project-link:hover { color:#2563eb; text-decoration:none; } .project-name { font-size:1.1rem; } .project-repository { color:var(--muted); font:500 .88rem 'JetBrains Mono',monospace; } .project-description { max-width:42ch; margin:7px 0 0; color:var(--muted); font-size:.86rem; line-height:1.55; } .score { color:#1d4ed8; font:600 1rem 'JetBrains Mono',monospace; } .mono { font-family:'JetBrains Mono',monospace; font-size:.96rem; }
...
.detail-hero { padding:34px; margin-bottom:20px; background:linear-gradient(135deg,#eff6ff,#ffffff 58%,#f0fdf4); border:1px solid #bfdbfe; border-radius:22px; } .detail-hero h1 { font-size:clamp(2rem,5vw,3.6rem); } .detail-hero .eyebrow { font-size:1rem; } .detail-hero .project-repository { font-size:1rem; } .detail-description { max-width:720px; color:#334155; font-size:1.15rem; line-height:1.65; }
.cards { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-bottom:20px; } .card { display:flex; flex-direction:column; padding:20px; background:var(--surface); border:1px solid var(--border); border-radius:14px; color:var(--muted); font-size:1.08rem; } .card-label { display:flex; align-items:baseline; flex-wrap:wrap; gap:8px; } .card-formula { margin-top:10px; color:#64748b; font:500 .72rem 'JetBrains Mono',monospace; } .card-formula-hint { margin:5px 0 0; color:#64748b; font-size:.78rem; } .card .score { display:block; margin-top:20px; font-size:2rem; color:#0f172a; }
.detail-table td { padding:18px 16px; font-size:1.05rem; } .detail-table td:first-child { width:35%; color:var(--muted); font-size:1.08rem; } .external-link { display:inline-flex; align-items:center; min-height:44px; margin-top:20px; padding:10px 14px; border:1px solid #bfdbfe; border-radius:10px; color:#1d4ed8; }
...
def index():
    ...
    return render_template_string(
        ...
        <div class='table-wrap'><table><thead><tr><th>排名</th><th>项目——所有者</th><th>热度分数</th><th>Stars</th><th>Forks</th></tr></thead><tbody>
        {% for project in projects %}<tr><td class='rank'>{% if loop.index == 1 %}<span class='medal medal-gold' aria-label='第1名'>🥇</span>{% elif loop.index == 2 %}<span class='medal medal-silver' aria-label='第2名'>🥈</span>{% elif loop.index == 3 %}<span class='medal medal-bronze' aria-label='第3名'>🥉</span>{% endif %}<span class='rank-number'>{{ '%02d'|format(loop.index) }}</span></td><td><a class='project-link' href='{{ url_for("project_detail", name=project.full_name or project.name) }}'><span class='project-name'>{{ project.chinese_name }}</span><span class='project-repository'>{{ project_owner_label(project) }}</span></a><p class='project-description'>{{ project.chinese_description }}</p></td>
...
def project_detail(name: str):
    ...
    return render_template_string(
        ...
        <section class='detail-hero'><p class='eyebrow'>项目详情</p><h1>{{ detail.chinese_name }}</h1><p class='project-repository'>{{ project_owner_label(detail) }}</p><p class='detail-description'>{{ detail.chinese_description }}</p><p class='detail-description'>{{ detail.description }}</p></section>
        <div class='cards'><div class='card'><div class='card-label'>热度分数</div><strong class='score'>{{ detail.hot_score }}</strong><span class='card-formula' title='P 为受欢迎度，R 为近期活跃度，S 为学生适配度'>H = 100 × (0.50P + 0.30R + 0.20S)</span><p class='card-formula-hint'>P 受欢迎度 · R 近期活跃度 · S 学生适配度</p></div>
